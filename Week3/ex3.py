...
def bar_chart_courses(students):
    ...
    plt.figure()
    plt.bar(list(courses.keys()), list(courses.values()), align='edge', edgecolor='black')
...
def course_chart_gender(students):
    ...
    plt.bar(xpos-0.2, list(females.values()), width=0.4, color='pink', edgecolor='red')
    plt.bar(xpos+0.2,list(males.values()), width=0.4, color='blue', edgecolor='black')
    plt.xticks(horizontalalignment='left', rotation=-45)
    plt.show()
